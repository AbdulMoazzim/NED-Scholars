...
export default function WebinarSeriesPage() {
  ...
  const fetchWebinars = async () => {
    ...
    try {
      ...
      if (allRes.success && allRes.data) {
        const completed = (allRes.data as Webinar[]).filter((w) => w.status === "completed");
  ...
  const trainingAreas = [
    { icon: <Target className="w-8 h-8" />, title: "Six Sigma & Lean Management", description: "Process optimization and quality management", color: "from-[#1164A3] to-[#68B9C4]" },
    { icon: <Briefcase className="w-8 h-8" />, title: "Project Management", description: "Advanced statistics and leadership skills", color: "from-[#68B9C4] to-[#82B4CC]" },
    { icon: <Zap className="w-8 h-8" />, title: "AI & Machine Learning", description: "Cutting-edge artificial intelligence applications", color: "from-[#82B4CC] to-[#B0A3B3]" },
    { icon: <Code className="w-8 h-8" />, title: "Embedded Systems", description: "Raspberry Pi, Arduino & IoT applications", color: "from-[#1164A3] to-[#82B4CC]" },
  ];

  const certificationBodies = [
    { name: "SME", fullName: "Society of Manufacturing Engineers", icon: <Award className="w-6 h-6" /> },
    { name: "ASEM", fullName: "American Society for Engineering Management", icon: <Award className="w-6 h-6" /> },
    { name: "ASME", fullName: "American Society of Mechanical Engineers", icon: <Award className="w-6 h-6" /> },
    { name: "IEOM Society", fullName: "Industrial Engineering & Operations Management", icon: <Award className="w-6 h-6" /> },
  ];

  const webinarFeatures = [
    { icon: <Users className="w-6 h-6" />, title: "Expert-Led Training", description: "Learn from industry professionals and experienced faculty" },
    { icon: <Award className="w-6 h-6" />, title: "Industry-Certified Programs", description: "Globally recognized certifications from reputable bodies" },
    { icon: <Laptop className="w-6 h-6" />, title: "Interactive Presentations", description: "Engaging sessions combining instruction and career guidance" },
    { icon: <Globe className="w-6 h-6" />, title: "Accessible Worldwide", description: "Join from anywhere, anytime that suits you" },
    { icon: <TrendingUp className="w-6 h-6" />, title: "Career-Focused Content", description: "Real-world skills that employers value" },
    { icon: <Lightbulb className="w-6 h-6" />, title: "Practical Application", description: "Hands-on learning with industry use cases" },
  ];
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4">What We Offer in Our Webinars</h2>
              <p className="text-gray-600 text-lg max-w-3xl mx-auto">
                Whether you&apos;re looking to upskill, earn certifications, or simply stay informed, our webinars deliver real-world value, directly to your screen.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center mb-12">
              <div>
                ...
                <p className="text-center text-gray-600 mb-6">High-demand areas such as:</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  {trainingAreas.map((area, index) => (
                    <Card key={index} className="hover:shadow-xl hover:border-[#1164A3] transition-all duration-300">
                      <CardContent className="p-6 text-center">
                        <div className={`w-16 h-16 bg-gradient-to-r ${area.color} rounded-2xl flex items-center justify-center text-white mx-auto mb-4`}>
                          {area.icon}
                        </div>
              ...
              <div className="relative rounded-2xl overflow-hidden shadow-2xl h-[460px]">
                ...
                <div className="absolute bottom-5 left-5 right-5">
                  <p className="text-white text-sm font-medium bg-black/30 rounded-xl px-4 py-2 backdrop-blur-sm">
                    Industry-leading content delivered live to your screen — from Six Sigma to AI
                  </p>
                </div>
              ...
              <Card className="shadow-xl border-[#82B4CC]/20 h-full">
                ...
                <CardContent>
                  <p className="text-gray-700 mb-6">
                    All certifications are recognized by reputable international bodies, ensuring your efforts stand out on a global stage:
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {certificationBodies.map((body, index) => (
                      <div key={index} className="flex items-start space-x-3 p-4 bg-[#82B4CC]/10 rounded-lg">
                        <div className="text-[#1164A3] flex-shrink-0">{body.icon}</div>
                        <div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4">Upcoming Webinars</h2>
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-[#1164A3]/65 to-transparent flex items-center">
                <div className="px-10 max-w-lg">
                  ...
                  <p className="text-white/85 text-sm leading-relaxed">
                    Register early — spots fill fast. Connect with experts and peers from across Pakistan and the world.
                  </p>
                </div>
              </div>
            </div>
            ...
            {loading ? (
              ...
              <Card className="border-[#82B4CC]/30">
                <CardContent className="p-12 text-center">
                  ...
                  <h3 className="text-xl font-semibold text-gray-800 mb-2">No Upcoming Webinars</h3>
                      ...
                      <div className="flex flex-col md:flex-row">
                        {webinar.images && webinar.images.length > 0 ? (
                          <div className="relative w-full md:w-64 h-48 md:h-auto overflow-hidden bg-gradient-to-br from-[#1164A3]/20 to-[#68B9C4]/20 flex-shrink-0">
                            <Image src={webinar.images[0].url} alt={webinar.images[0].alt || webinar.title} fill className="object-cover group-hover:scale-110 transition-transform duration-500" />
                            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                            <div className="absolute top-3 left-3 flex flex-wrap gap-2">
                              <Badge className="bg-black/70 text-white border-0"><Video className="w-3 h-3 mr-1" />{webinar.platform || "Webinar"}</Badge>
                              <Badge className="bg-black/70 text-white border-0">{webinar.status}</Badge>
                            </div>
                          </div>
                        ) : (
                          <div className={`relative w-full md:w-64 h-48 md:h-auto bg-gradient-to-r ${getWebinarColor(index)} flex items-center justify-center flex-shrink-0`}>
                            ...
                            <div className="absolute top-3 left-3 flex flex-wrap gap-2">
                              <Badge className="bg-white/20 text-white border-white/30"><Video className="w-3 h-3 mr-1" />{webinar.platform || "Webinar"}</Badge>
                              <Badge className="bg-white/20 text-white border-white/30">{webinar.status}</Badge>
                            </div>
                          </div>
                        )}
                        <div className="flex-1 p-8">
                          <h3 className="text-xl font-bold text-gray-800 mb-2 group-hover:text-[#1164A3] transition-colors">{webinar.title}</h3>
                          <p className="text-gray-600 mb-3 line-clamp-2">{webinar.description}</p>
                          <div className="flex flex-wrap gap-4 text-sm text-gray-600 mb-4">
                            <span className="flex items-center"><Calendar className="w-4 h-4 mr-1" />{formatDate(webinar.date)}</span>
                            <span className="flex items-center"><Clock className="w-4 h-4 mr-1" />{formatTime(webinar.date)}</span>
                            <span className="flex items-center"><Video className="w-4 h-4 mr-1" />{calculateDuration(webinar.date, webinar.endDate || undefined)}</span>
                            {webinar.maxParticipants && (
                              <span className="flex items-center text-[#68B9C4] font-medium">
                                <Users className="w-4 h-4 mr-1" />{webinar.maxParticipants - webinar.attendees.length} spots available
                              </span>
                            )}
                          </div>
                          <button
                            onClick={(e) => { e.stopPropagation(); router.push(`/register/webinar-attendee?webinarId=${webinar.id}`); }}
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4">Previous Webinar Recordings</h2>
              <p className="text-gray-600 text-lg">Access our library of past webinars and continue learning</p>
            </div>
            ...
            {loading ? (
              ...
              <Card className="border-[#82B4CC]/30">
                <CardContent className="p-12 text-center">
                  ...
                  <h3 className="text-xl font-semibold text-gray-800 mb-2">No Previous Webinars</h3>
                  <p className="text-gray-600">Past webinar recordings will appear here</p>
                </CardContent>
              </Card>
            ) : (
              <>
                    ...
                    >
                      {webinar.images && webinar.images.length > 0 ? (
                        <div className="relative overflow-hidden h-48">
                          ...
                          <Badge className="absolute top-3 right-3 bg-black/70 text-white border-0"><Users className="w-3 h-3 mr-1" />{webinar.attendees.length}</Badge>
                          <Badge className="absolute bottom-3 right-3 bg-black/70 text-white border-0">{calculateDuration(webinar.date, webinar.endDate || undefined)}</Badge>
                        </div>
                      ) : (
                      ...
                      <div className="p-5">
                        <div className="flex items-center justify-between mb-2">
                          <Badge className="bg-[#82B4CC] text-white">{webinar.platform || "Webinar"}</Badge>
                          <span className="text-sm font-semibold text-gray-700">{webinar.status}</span>
                        </div>
                        <h4 className="font-bold text-gray-800 mb-2 line-clamp-2 group-hover:text-[#1164A3] transition-colors">{webinar.title}</h4>
                        <p className="text-sm text-gray-600 mb-3 line-clamp-2">{webinar.description}</p>
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-gray-500 flex items-center"><Calendar className="w-3 h-3 mr-1" />{formatDate(webinar.date)}</span>
                          <span className="text-[#1164A3] hover:text-[#68B9C4] text-sm font-medium flex items-center">Watch <ArrowRight className="w-4 h-4 ml-1" /></span>
                        </div>
                      </div>
                    </Card>
                  ))}
                ...
                {previousWebinars.length > 6 && viewMoreData > 6 && (
                  <div className="text-center mt-8">
                    ...
                    >
                      <Video className="w-5 h-5 mr-2" />View All Webinars<ArrowRight className="w-5 h-5 ml-2" />
                    </button>
                  </div>
                )}
              </>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4">Benefits of Our Webinar Series</h2>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
              ...
              <div className="relative rounded-2xl overflow-hidden shadow-2xl h-[360px]">
                ...
                <div className="absolute bottom-5 left-5 right-5">
                  <p className="text-white text-sm font-medium bg-black/30 rounded-xl px-4 py-2 backdrop-blur-sm">
                    Learn from anywhere and earn credentials that matter worldwide
                  </p>
                </div>
                <div className="absolute -bottom-3 -right-3 w-full h-full rounded-2xl border-4 border-[#68B9C4]/30 -z-10" />
              </div>
          ...
          <div className="max-w-4xl mx-auto">
            <Card className="shadow-2xl bg-gradient-to-r from-[#82B4CC]/20 to-[#B0A3B3]/20 border-l-4 border-[#1164A3]">
              <CardContent className="p-8 md:p-12 text-center">
                ...
                <h3 className="text-2xl md:text-3xl font-bold text-gray-800 mb-6">Learn From Anywhere, Grow Everywhere</h3>
                <p className="text-lg text-gray-700">
                  Join thousands of students and professionals who are advancing their careers through NED Scholars Webinar Series. Your future starts with the next webinar you attend.
                </p>
              </CardContent>
            </Card>
          </div>
